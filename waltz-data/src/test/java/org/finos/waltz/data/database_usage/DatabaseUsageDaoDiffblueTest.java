...
class DatabaseUsageDaoDiffblueTest {
  ...
  void testFindByDatabaseId() throws DataAccessException {
    ...
    Collection<DatabaseUsage> actualFindByDatabaseIdResult =
        new DatabaseUsageDao(dsl).findByDatabaseId(1L);
   ...
   *
   * <ul>
   *   <li>Given {@link DatabaseUsageDao#DatabaseUsageDao(DSLContext)} with dsl is {@link
   *       DSLContext}.
   * </ul>
   *
   * <p>Method under test: {@link DatabaseUsageDao#findByEntityReference(EntityReference)}
  ...
  @Test
  @DisplayName(
      "Test findByEntityReference(EntityReference); given DatabaseUsageDao(DSLContext) with dsl is DSLContext")
  @Tag("ContributionFromDiffblue")
  @ManagedByDiffblue
  @MethodsUnderTest({"Collection DatabaseUsageDao.findByEntityReference(EntityReference)"})
  void testFindByEntityReference_givenDatabaseUsageDaoWithDslIsDSLContext()
    ...
    DSLContext dsl = mock(DSLContext.class);
    when(dsl.select((SelectFieldOrAsterisk[]) Mockito.any())).thenReturn(selectSelectStep);
    DatabaseUsageDao databaseUsageDao = new DatabaseUsageDao(dsl);
