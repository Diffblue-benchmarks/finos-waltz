...
class SqlServerMeasurableSearchDiffblueTest {
   ...
   *
   * <p>Method under test: {@link SqlServerMeasurableSearch#searchFullText(DSLContext,
   * EntitySearchOptions)}
   */
  @Test
  ...
  @ManagedByDiffblue
  @MethodsUnderTest({
    "List SqlServerMeasurableSearch.searchFullText(DSLContext, EntitySearchOptions)"
  })
  void testSearchFullText() throws DataAccessException {
    ...
    SelectLimitPercentStep<MeasurableRecord> selectLimitPercentStep =
        mock(SelectLimitPercentStep.class);
    when(selectLimitPercentStep.fetch(Mockito.<RecordMapper<MeasurableRecord, Object>>any()))
        .thenReturn(new ArrayList<>());

    SelectConditionStep<MeasurableRecord> selectConditionStep = mock(SelectConditionStep.class);
    when(selectConditionStep.limit(anyInt())).thenReturn(selectLimitPercentStep);

    SelectConditionStep<MeasurableRecord> selectConditionStep2 = mock(SelectConditionStep.class);
    when(selectConditionStep2.and(Mockito.<Condition>any())).thenReturn(selectConditionStep);

    SelectWhereStep<MeasurableRecord> selectWhereStep = mock(SelectWhereStep.class);
    when(selectWhereStep.where(Mockito.<SQL>any())).thenReturn(selectConditionStep2);

    DSLContext dsl = mock(DSLContext.class);
    when(dsl.selectFrom(Mockito.<Table<MeasurableRecord>>any())).thenReturn(selectWhereStep);

    Builder builderResult = ImmutableEntitySearchOptions.builder();

    // Act
    List<Measurable> actualSearchFullTextResult =
        sqlServerMeasurableSearch.searchFullText(
            dsl,
            builderResult
