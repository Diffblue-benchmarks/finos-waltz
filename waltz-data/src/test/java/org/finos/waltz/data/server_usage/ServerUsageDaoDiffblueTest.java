...
class ServerUsageDaoDiffblueTest {
  ...
  @Test
  @DisplayName(
      "Test findForEntity(EntityReference); given one; when EntityReference id() return one; then calls id()")
  @Tag("ContributionFromDiffblue")
  @ManagedByDiffblue
  @MethodsUnderTest({"List ServerUsageDao.findForEntity(EntityReference)"})
  void testFindForEntity_givenOne_whenEntityReferenceIdReturnOne_thenCallsId()
  ...
  void testFindByServerId() throws DataAccessException {
    ...
    // Act
    Collection<ServerUsage> actualFindByServerIdResult = serverUsageDao.findByServerId(1L);
