...
class AssessmentGeneratorDiffblueTest {
   ...
   * </ul>
   * <p>
   * Method under test: {@link AssessmentGenerator#remove(ApplicationContext)}
   */
  @Test
  ...
  void testRemove_givenDeleteConditionStepExecuteReturnOne_thenCallsExecute()
    ...
    when(defaultDSLContext.deleteFrom(Mockito.<Table<AssessmentRatingRecord>>any())).thenReturn(deleteUsingStep);
    AnnotationConfigApplicationContext ctx = mock(AnnotationConfigApplicationContext.class);
   ...
   */
  @Test
  @DisplayName("Test remove(ApplicationContext); then throw RuntimeException")
  @Tag("MaintainedByDiffblue")
  @MethodsUnderTest({"boolean AssessmentGenerator.remove(ApplicationContext)"})
  void testRemove_thenThrowRuntimeException() throws BeansException {
    // Arrange
    AssessmentGenerator assessmentGenerator = new AssessmentGenerator();
    DeleteUsingStep<AssessmentRatingRecord> deleteUsingStep = mock(DeleteUsingStep.class);
    when(deleteUsingStep.where(Mockito.<Condition>any()))
        .thenThrow(new RuntimeException(DatabaseSoftwarePackages.SAMPLE_DATA_PROVENANCE));
    DefaultDSLContext defaultDSLContext = mock(DefaultDSLContext.class);
    when(defaultDSLContext.deleteFrom(Mockito.<Table<AssessmentRatingRecord>>any())).thenReturn(deleteUsingStep);
    AnnotationConfigApplicationContext ctx = mock(AnnotationConfigApplicationContext.class);
    when(ctx.getBean(Mockito.<Class<DSLContext>>any())).thenReturn(defaultDSLContext);
    ...
    // Act and Assert
    assertThrows(RuntimeException.class, () -> assessmentGenerator.remove(ctx));
