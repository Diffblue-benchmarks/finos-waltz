...
class ChangeInitiativeGeneratorDiffblueTest {
   ...
   * </ul>
   * <p>
   * Method under test: {@link ChangeInitiativeGenerator#remove(ApplicationContext)}
   */
  @Test
   ...
   *   <li>Then calls {@link DeleteConditionStep#and(Condition)}.</li>
   * </ul>
   * <p>
   * Method under test: {@link ChangeInitiativeGenerator#remove(ApplicationContext)}
   */
  @Test
  @DisplayName("Test remove(ApplicationContext); given DeleteConditionStep execute() return one; then calls and(Condition)")
  @Tag("MaintainedByDiffblue")
  @MethodsUnderTest({"boolean ChangeInitiativeGenerator.remove(ApplicationContext)"})
  void testRemove_givenDeleteConditionStepExecuteReturnOne_thenCallsAnd() throws DataAccessException, BeansException {
    // Arrange
    ChangeInitiativeGenerator changeInitiativeGenerator = new ChangeInitiativeGenerator();
    ...
    when(defaultDSLContext.deleteFrom(Mockito.<Table<EntityRelationshipRecord>>any())).thenReturn(deleteUsingStep);
    AnnotationConfigApplicationContext ctx = mock(AnnotationConfigApplicationContext.class);
    ...
    verify(deleteConditionStep2, atLeast(1)).execute();
