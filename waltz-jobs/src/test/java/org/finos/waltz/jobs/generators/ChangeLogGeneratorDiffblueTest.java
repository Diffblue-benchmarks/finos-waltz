...
class ChangeLogGeneratorDiffblueTest {
  /**
   * Test {@link ChangeLogGenerator#remove(ApplicationContext)}.
   * <ul>
   *   <li>Given {@link PreparedStatement} {@link PreparedStatement#execute()} return {@code true}.</li>
   *   <li>Then return {@code false}.</li>
   * </ul>
   * <p>
   * Method under test: {@link ChangeLogGenerator#remove(ApplicationContext)}
   */
  @Test
  @DisplayName("Test remove(ApplicationContext); given PreparedStatement execute() return 'true'; then return 'false'")
  @Tag("MaintainedByDiffblue")
  @MethodsUnderTest({"boolean ChangeLogGenerator.remove(ApplicationContext)"})
  void testRemove_givenPreparedStatementExecuteReturnTrue_thenReturnFalse() throws SQLException, BeansException {
    ...
    // Act
    boolean actualRemoveResult = changeLogGenerator.remove(ctx);

    // Assert
    verify(connection).prepareStatement(eq("delete from \"change_log\""));
