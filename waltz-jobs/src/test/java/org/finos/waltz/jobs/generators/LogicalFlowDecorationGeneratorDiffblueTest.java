...
class LogicalFlowDecorationGeneratorDiffblueTest {
  /**
   * Test {@link LogicalFlowDecorationGenerator#remove(ApplicationContext)}.
   * <ul>
   *   <li>Given {@link DeleteConditionStep} {@link Query#execute()} return one.</li>
   *   <li>Then return {@code false}.</li>
   * </ul>
   * <p>
   * Method under test: {@link LogicalFlowDecorationGenerator#remove(ApplicationContext)}
   */
  @Test
  @DisplayName("Test remove(ApplicationContext); given DeleteConditionStep execute() return one; then return 'false'")
  @Tag("MaintainedByDiffblue")
  @MethodsUnderTest({"boolean LogicalFlowDecorationGenerator.remove(ApplicationContext)"})
  void testRemove_givenDeleteConditionStepExecuteReturnOne_thenReturnFalse()
      throws DataAccessException, BeansException {
    // Arrange
    LogicalFlowDecorationGenerator logicalFlowDecorationGenerator = new LogicalFlowDecorationGenerator();
    ...
    when(defaultDSLContext.deleteFrom(Mockito.<Table<LogicalFlowDecoratorRecord>>any())).thenReturn(deleteUsingStep);
    ...
    // Act
    boolean actualRemoveResult = logicalFlowDecorationGenerator.remove(ctx);
