...
class OrgUnitGeneratorDiffblueTest {
  /**
   * Test {@link OrgUnitGenerator#remove(ApplicationContext)}.
   * <ul>
   *   <li>Given {@link DeleteUsingStep} {@link Query#execute()} return one.</li>
   *   <li>Then calls {@link Query#execute()}.</li>
   * </ul>
   * <p>
   * Method under test: {@link OrgUnitGenerator#remove(ApplicationContext)}
   */
  @Test
  @DisplayName("Test remove(ApplicationContext); given DeleteUsingStep execute() return one; then calls execute()")
  @Tag("MaintainedByDiffblue")
  @MethodsUnderTest({"boolean OrgUnitGenerator.remove(ApplicationContext)"})
  void testRemove_givenDeleteUsingStepExecuteReturnOne_thenCallsExecute() throws DataAccessException, BeansException {
    // Arrange
    OrgUnitGenerator orgUnitGenerator = new OrgUnitGenerator();
    DeleteUsingStep<OrganisationalUnitRecord> deleteUsingStep = mock(DeleteUsingStep.class);
    when(deleteUsingStep.execute()).thenReturn(1);
    DefaultDSLContext defaultDSLContext = mock(DefaultDSLContext.class);
    when(defaultDSLContext.deleteFrom(Mockito.<Table<OrganisationalUnitRecord>>any())).thenReturn(deleteUsingStep);
    ...
    // Act
    boolean actualRemoveResult = orgUnitGenerator.remove(ctx);
  ...
  void testRemove_givenPreparedStatementExecuteReturnTrue_thenCallsPrepareStatement()
    ...
    // Assert
    verify(connection).prepareStatement(eq("delete from \"organisational_unit\""));
