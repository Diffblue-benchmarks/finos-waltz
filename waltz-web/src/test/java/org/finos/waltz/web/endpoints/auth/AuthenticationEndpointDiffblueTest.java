...
class AuthenticationEndpointDiffblueTest {
   ...
   * <p>
   * Method under test: {@link AuthenticationEndpoint#register()}
   */
  @Test
  ...
  void testRegister() {
    ...
    // Act
    (new AuthenticationEndpoint(userService, new UserRoleService(userRoleDao2, roleDao, personDao, changeLogService,
        personService, new SettingsService(settingsDao3, new ArrayList<>())), settingsService, null)).register();

    // Assert
    verify(settingsDao).getByName(eq("server.authentication.filter"));
  }
}
